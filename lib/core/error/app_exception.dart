...
class NoInternetError extends AppException {
  NoInternetError()
      : super(503, 'No internet connection'); // Service Unavailable
}
...
class HandshakeError extends AppException {
  HandshakeError()
      : super(526, 'SSL Handshake failed'); // Invalid SSL certificate
}

// Session expired error (custom code)
class SessionExpiry extends AppException {
  SessionExpiry()
      : super(401, 'Session has expired'); // Unauthorized (session expired)
}

// Unknown error (fallback for undefined errors)
class UnknownError extends AppException {
  UnknownError() : super(520, 'An unknown error occurred'); // Unknown error
}
...
class NotFoundError extends AppException {
  NotFoundError({int? statusCode, String? message})
...
class BadRequestError extends AppException {
  BadRequestError({int? statusCode, String? message})
...
class UnprocessableEntityError extends AppException {
  UnprocessableEntityError({int? statusCode, String? message})
...
class ConflictError extends AppException {
  ConflictError({int? statusCode, String? message})
      : super(statusCode ?? 409, message ?? 'Conflict');
}
...
class PreconditionFailedError extends AppException {
  PreconditionFailedError({int? statusCode, String? message})
      : super(statusCode ?? 412, message ?? 'Precondition failed');
}

// User Exists error (422 - user already exists, conflict)
class UserExistsError extends AppException {
  UserExistsError({int? statusCode, String? message})
      : super(statusCode ?? 422, message ?? 'User already exists');
}

class ValidationError extends AppException {
  ValidationError({int? statusCode, String? message})
      : super(statusCode ?? 0, message ?? 'Something went wrong!');
}
...
class MethodNotAllowedError extends AppException {
  MethodNotAllowedError({int? statusCode, String? message})
      : super(
            statusCode ?? 405,
            message ??
                'Method Not Allowed: The HTTP method used is not supported for the requested resource.');
}
